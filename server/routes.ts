...
const handleSupportIssue = async (
  ...
  let tgUser = await storage.getTelegramUser(userId) || await storage.getTelegramUserByChatId(chatId.toString());
  if (!tgUser && userId) {
  ...
  const displayTicketId = ticketId > 0 ? (ticketId < 2000 ? ticketId + 2000 : ticketId) : 2001;
  const caption = `<tg-emoji emoji-id="5260535596941582167">💬</tg-emoji> <b>${issueTitle}</b> (Ticket #${displayTicketId})\n\n` +
    `Please <b>type your message or problem details below in this chat</b> (or attach a screenshot).\n\n` +
    `<blockquote>Order ID:\n` +
    `Payment method / Amount:\n` +
    `Problem details:</blockquote>\n\n` +
    `<i>Or click contact admin below to message directly: @${cleanUsername}</i>`;
...
async function processCryptomusInvoiceCreation(targetBot: TelegramBot, chatId: number, tgUser: any, amount: number) {
  ...
  try {
    ...
    if (response.data.result) {
      ...
      const showWebBtnSetting = await storage.getSetting('SHOW_CRYPTOMUS_WEB_BUTTON');
      const showWebBtn = showWebBtnSetting?.value !== 'false';

      const inline_keyboard: any[][] = [];
      if (showWebBtn) {
        inline_keyboard.push([{ text: 'Go to payment', url: paymentData.url, icon_custom_emoji_id: '5373123633415695601' }]);
      }
      inline_keyboard.push([{ text: 'Check payment', callback_data: `check_payment_${newPayment.id}`, icon_custom_emoji_id: '6010111371251815589' }]);
...
async function processCryptomusBep20InvoiceCreation(targetBot: TelegramBot, chatId: number, tgUser: any, amount: number, messageIdToEdit?: number) {
  ...
  try {
    ...
    if (response.data && response.data.result) {
      ...
      const keyboard: any[][] = [];
      if (showWebBtn) {
        keyboard.push([{ text: 'Pay on Cryptomus Web', url: paymentData.url, icon_custom_emoji_id: '5373123633415695601' }]);
      }
      keyboard.push(
        [{ text: 'Generate QR Code', callback_data: `gen_qr_bep20_${newPayment.id}`, icon_custom_emoji_id: '5309771942381785364' }],
...
async function processCryptomusTrc20InvoiceCreation(targetBot: TelegramBot, chatId: number, tgUser: any, amount: number, messageIdToEdit?: number) {
  ...
  if (!apiKey || !merchantId) {
    const staticWallet = (await storage.getSetting('TRC20_WALLET_ADDRESS'))?.value || "TJR7q1c8k5v74B8F91M5B1mzpml3x9k";
    ...
    const keyboard = [
      ...
      [{ text: 'Copy Wallet Address', copy_text: { text: staticWallet }, icon_custom_emoji_id: '5231102735817918643' }],
  ...
  try {
    ...
    if (response.data && response.data.result) {
      const paymentData = response.data.result;
      const trc20Address = paymentData.address || (await storage.getSetting('TRC20_WALLET_ADDRESS'))?.value || "TJR7q1c8k5v74B8F91M5B1mzpml3x9k";
      ...
      const responseMsg = `<tg-emoji emoji-id="5936189134342199863">💰</tg-emoji> You need to pay <b>${amount.toFixed(0)} USDT</b> \n\n` +
        `<b>Coin:</b> USDT <tg-emoji emoji-id="5201692367437974073">💵</tg-emoji>\n` +
        `<b>Network:</b> TRC20 (Cryptomus) <tg-emoji emoji-id="5936189134342199863">💰</tg-emoji>\n\n` +
      ...
      keyboard.push(
        [{ text: 'Generate QR Code', callback_data: `gen_qr_trc20_${newPayment.id}`, icon_custom_emoji_id: '5309771942381785364' }],
        [{ text: 'Copy Wallet Address', copy_text: { text: trc20Address }, icon_custom_emoji_id: '5231102735817918643' }],
        [{ text: 'Check payment', callback_data: `check_payment_${newPayment.id}`, icon_custom_emoji_id: '5386367538735104399' }],
        [{ text: 'Change Network', callback_data: 'add_funds', icon_custom_emoji_id: '5976535107933050770' }]
      );
    ...
    console.error('Cryptomus TRC20 creation error:', err.response?.data || err.message);
    targetBot.sendMessage(chatId, "❌ Failed to create Cryptomus TRC20 invoice. Please try again later.");
  }
}
  ...
  targetBot.on('callback_query', async (query) => {
    try {
      ...
      if (data.startsWith('gen_qr_')) {
        ...
        const payment = await storage.getPayment(paymentId);
        let walletAddress = payment?.txid;
        if (!walletAddress || walletAddress.trim() === '') {
          if (method === 'trc20') {
            walletAddress = (await storage.getSetting('TRC20_WALLET_ADDRESS'))?.value || "TJR7q1c8k5v74B8F91M5B1mzpml3x9k";
      ...
      if (data === 'payment_trc20') {
        ...
        const wallet = (await storage.getSetting('TRC20_WALLET_ADDRESS'))?.value || "TJR7q1c8k5v74B8F91M5B1mzpml3x9k";
      ...
      if (data.startsWith('trc20_amount_')) {
        ...
        await processCryptomusTrc20InvoiceCreation(targetBot, chatId, tgUser, amount, query.message?.message_id);
        return;
      }
  ...
  return `<tg-emoji emoji-id="5949584381424178413">✅</tg-emoji> <b>Your Ticket Has Been Submitted!</b>\n\n` +
    ...
    targetBot.on('message', async (msg) => {
        ...
        if (tgUser?.lastAction?.startsWith('awaiting_support_')) {
          ...
          if (msg.photo && msg.photo.length > 0) {
            ...
            try {
              ...
              const fileName = `support_screenshot_${ticketId || Date.now()}_${Date.now()}.jpg`;
          ...
          try {
            ...
            if (ticketId > 0) {
              ...
              existingTicket = ticketRes[0];
            }

            let messagesList: Array<{ sender: string; text: string; timestamp: string }> = [];
            if (existingTicket?.messages) {
              try {
                messagesList = JSON.parse(existingTicket.messages);
              } catch (e) {
                messagesList = [];
              }
            } else if (existingTicket?.details) {
              messagesList.push({
                sender: 'user',
                text: existingTicket.details,
                timestamp: existingTicket.createdAt ? new Date(existingTicket.createdAt).toISOString() : new Date().toISOString()
              });
            }

            const currentMsgText = supportDetailsText || (relativeUrl ? '[Screenshot Attached]' : 'Support Details Provided');
            messagesList.push({
              sender: 'user',
              text: currentMsgText,
              timestamp: new Date().toISOString()
            ...
            if (isNaN(ticketId) || ticketId === 0 || !existingTicket) {
              const ticket = await storage.createSupportTicket({
                telegramUserId: tgUser.id,
            ...
            // Send push notification & socket event to Admin Dashboard ONLY NOW after details are entered!
            sendAdminPushNotification({
              title: `🆘 Support Ticket Received (#${displayTicketId})`,
              body: `@${tgUser.username || userId}: ${currentMsgText.substring(0, 100)}`
            }).catch(() => {});

            io.emit('admin_notification', {
              type: 'support',
              title: `Support Ticket Submitted (#${displayTicketId})`,
              message: `User @${tgUser.username || userId} submitted ticket #${displayTicketId}: ${currentMsgText}`,
              data: { ticketId: displayTicketId, realTicketId: ticketId, userId, details: currentMsgText, attachmentUrl: relativeUrl }
            });

            // Forward to Admin Chat ID if configured
            const adminSetting = await storage.getSetting('ADMIN_CHAT_ID');
            if (adminSetting?.value) {
              const adminMsgText = `🆘 <b>New Support Ticket (#${displayTicketId})</b>\n\n` +
                `User: @${tgUser.username || tgUser.firstName || userId} (<code>${userId}</code>)\n` +
                `Details: ${escapeHTML(currentMsgText)}`;
            ...
            // Reply to user with Custom Emoji ticket conversation thread
            const confirmationMsg = formatTicketMessageThread(displayTicketId, 'open', messagesList);
      ...
      const isNumericInput = Boolean(
        ...
        !tgUser?.lastAction?.startsWith('awaiting_support_') &&
      ...
      if ((isBinanceState || isBinanceReply || isNumericInput) && !isDepositAmountState) {
      ...
      if (tgUser?.lastAction?.includes('_auth_pass_await')) {
        ...
        await processCryptomusTrc20InvoiceCreation(targetBot, chatId, tgUser, amount);
...
BackupService.startBackupScheduler().catch(err => console.error("Backup scheduler failed to start:", err));
      ...
      if (updated && updated.userTelegramId) {
        try {
          ...
          if (targetBot) {
            ...
            if (!isNaN(chatId)) {
              const displayId = updated.id < 2000 ? updated.id + 2000 : updated.id;
              if (status === 'resolved') {
                await targetBot.sendMessage(
                  chatId,
                  `<tg-emoji emoji-id="5949584381424178413">✅</tg-emoji> <b>Support Ticket #${displayId} Resolved</b>\n\n` +
                  `<tg-emoji emoji-id="5850383023572259486">🎫</tg-emoji> <b>Ticket ID:</b> <code>#${displayId}</code>\n` +
                  `<tg-emoji emoji-id="5805188079148863343">🕒</tg-emoji> <b>Status:</b> <b>Resolved & Closed</b>\n\n` +
                  `Your support request regarding <b>${escapeHTML(updated.issueType)}</b> has been marked as <b>Resolved</b> by our support team.\n\n` +
                  `<tg-emoji emoji-id="5404617696589390973">✨</tg-emoji> Thank you for reaching out! If you still need help, feel free to open a new ticket anytime.`,
                  { parse_mode: 'HTML' }
                ).catch(() => {});
              } else if (status === 'closed') {
                await targetBot.sendMessage(
                  chatId,
                  `<tg-emoji emoji-id="5215570077876756627">❌</tg-emoji> <b>Support Ticket #${displayId} Closed</b>\n\n` +
                  `<tg-emoji emoji-id="5850383023572259486">🎫</tg-emoji> <b>Ticket ID:</b> <code>#${displayId}</code>\n` +
                  `<tg-emoji emoji-id="5805188079148863343">🕒</tg-emoji> <b>Status:</b> <b>Closed</b>\n\n` +
                  `Your support request regarding <b>${escapeHTML(updated.issueType)}</b> has been marked as <b>Closed</b> by our support team.\n\n` +
                  `<tg-emoji emoji-id="5404617696589390973">✨</tg-emoji> Thank you for using our support center!`,
                  { parse_mode: 'HTML' }
                ).catch(() => {});
              } else if (status === 'open' || status === 'in_progress') {
                await targetBot.sendMessage(
                  chatId,
                  `<tg-emoji emoji-id="5260535596941582167">💬</tg-emoji> <b>Support Ticket #${displayId} Reopened</b>\n\n` +
  ...
  // --- Support Ticket Admin Reply Endpoint ---
  app.post("/api/support-tickets/:id/reply", isAuth, async (req, res) => {
    try {
      ...
      const ticketList = await db.select().from(supportTickets).where(eq(supportTickets.id, id));
      ...
      const targetBot = getBotInstance();
      if (targetBot && updatedTicket.userTelegramId) {
        const chatId = parseInt(updatedTicket.userTelegramId, 10);
        if (!isNaN(chatId)) {
          const displayTicketId = updatedTicket.id < 2000 ? updatedTicket.id + 2000 : updatedTicket.id;
          const formattedThreadMsg = formatTicketMessageThread(displayTicketId, updatedTicket.status, messagesList);
          ...
          await storage.updateTelegramUserByChatId(updatedTicket.userTelegramId, {
            lastAction: `awaiting_support_details_${updatedTicket.id}`
          }).catch(() => {});
        }
      }
      ...
      io.emit('admin_notification', {
        ...
        title: `Reply Sent to Ticket #${updatedTicket.id}`,
        message: `Admin replied to ticket #${updatedTicket.id}`
      });
